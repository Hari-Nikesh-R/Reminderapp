  ...
  fun print(text: String) {
   ...
   val interfaceCount = usbDevice?.interfaceCount?:0
    ...
    if (usbInterface == null || usbEndpoint == null) {
      // Handle error: Printer interface or endpoint not found
      Log.e("Device List", "Printer interface or endpoint not found")
      return
    }

    // Claim the interface
    if (usbDeviceConnection?.claimInterface(usbInterface, true) == true) {
      // Optional: Perform printer initialization (e.g., soft reset)
      val requestType = UsbConstants.USB_DIR_OUT or UsbConstants.USB_TYPE_CLASS
//      val requestType = UsbConstants.USB_DIR_OUT or UsbConstants.USB_TYPE_CLASS or UsbConstants.USB_INTERFACE_SUBCLASS
      ...
      val controlResult = usbDeviceConnection?.controlTransfer(
        requestType, request, value, index?:0,
        null, 0, 0, timeout
      )
      ...
      if (controlResult != null) {
        when {
          controlResult >= 0 -> { // Success - proceed with bulk transfer
            ...
            Log.e("Device List", "Printer initialization failed (Code: $controlResult)") // Abort operation, show error to user
            return
          }
        }
      }

      // Perform bulk transfer with the correct OUT endpoint
      ...
      // Handle interface claim failure
      Log.i("Devices list", "Failed to claim interface")
      openUsbDevice()
    ...
    usbDevice?.let { device ->
      try {
        ...
        if (connection != null) {
          ...
          // ... your code to communicate with the device ...
